...
import { faUniversity, faMapMarkerAlt, faPhone, faBook, faImage, faUserShield, faInfoCircle, faSpinner, faUpload } from '@fortawesome/free-solid-svg-icons';
...
const UniversityRegister = () => {
  ...
  const progress = useMemo(() => {
    const filledFields = requiredFields.filter((field) => formData[field] && formData[field] !== '').length;
  ...
  const validateForm = () => {
    ...
    if (formData.brochure && !['application/pdf'].includes(formData.brochure.type)) {
        ...
        <div onSubmit={handleSubmit} className="space-y-12">
                  ...
                  <div key={field.name} className="space-y-1">
                    {field.type === 'file' ? (
                      <div>
                        ...
                        {formData[field.name] && (
                          <p className="text-sm text-gray-500 mt-1">Selected: {formData[field.name].name}</p>
                        )}
                      </div>
                    ) : (
                      <FormInput
          ...
          <div className="flex justify-center">
            ...
            >
              {isSubmitting ? (
                <>
                  <FontAwesomeIcon icon={faSpinner} className="animate-spin mr-3 h-5 w-5 text-[#3656E5]" />
                  Submitting...
                </>
              ) : (
                'Submit Registration'
